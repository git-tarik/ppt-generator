...
def generate_presentation(template_content: bytes, slide_plan: dict) -> io.BytesIO:
    """
    Generates a PPTX file by cloning slides from the template.
    Strictly follows slide plan count.
    """
    try:
    ...
    # DEV DEBUG: Verify Loop Execution
    # print(f"PLAN_COUNT: {len(slides_data)}")
    
    # 3. Generate New Slides
    # Loop ONLY over slide_plan["slides"]
    for i, slide_data in enumerate(slides_data):
        print(f"CREATING SLIDE {i}") # Mandatory Debug Print as requested
        ...
        # Update
        update_slide_content(new_slide, slide_data)
